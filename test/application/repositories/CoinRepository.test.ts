...
describe('CoinRepository', () => {
  ...
  it('should update coin status', () => {
    ...
    coinRepo.upsertCoin('xch1234', coin);
    coinRepo.updateCoinStatus('xch1234', coin.coinId, 'spent', 11);
    const coins = coinRepo.getCoins('xch1234');
    expect(coins[0].status).toBe('spent');
    expect(coins[0].synced_height).toBe(11);
  });
  ...
  it('should get pending coins', () => {
    ...
    const coin2 = {
      ...
      parent_coin_info: Buffer.from('eeff00', 'hex'),
      puzzle_hash: Buffer.from('223344', 'hex'),
      amount: BigInt(2000),
      synced_height: 12,
      status: 'unspent',
    };
